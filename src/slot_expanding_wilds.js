...
async function start() {
  const canvas = document.getElementById("slotExpandingWildsCanvas");
  ...
  const symbolNames = ["symbol_1", "symbol_2", "symbol_3", "symbol_4", "symbol_5",
                       "symbol_6", "symbol_7", "symbol_8", "symbol_9"
                      ];
  const symbolTextures = {};
  for (const name of symbolNames) {
    symbolTextures[name] = await Assets.load(
      `/assets/slot_expanding_wilds/${name}.png`
    );
  }
  // create reels
  const reels = [];
  ...
  spinButton.on("pointerdown", async () => {
    ...
    try {
      const response = await fetch("/api/slot_expanding_wilds/spin", {
        method: "POST",
      ...
      const isTwoStage = reelData.length === 2 ? true : false;
      // const isTwoStage = Array.isArray(reelData?.[0]?.[0]);
      // single-stage: reelData[0][0] is a number
      // two-stage: reelData[0][0] is an array (a reel)
      // console.log(isTwoStage);
      let stageExpanded = null;
      let stageOriginal = null;

      if (isTwoStage) {
        stageExpanded = reelData[0]; // expanded with wilds
        stageOriginal = reelData[1]; // original with wild symbol visible
      } else {
        stageOriginal = reelData[0]; // the only stage
      }
      ...
      reels.forEach((reel, i) => {
        setTimeout(() => {
          reel.onStopComplete = async () => {
            ...
            // --- If 2-stage: run expanding wild animation ---
            if (stageExpanded) {
              // find which reels contain wild in the ORIGINAL stage
              const wildReels = [];
              for (let col = 0; col < reelCols; col++) {
                if (stageOriginal[col]?.includes(1)) wildReels.push(col);
              }

              // animate expansions (sequential looks good)
              for (const col of wildReels) {
                await reels[col].playExpandWild(stageExpanded[col]);
            ...
            if (Array.isArray(scatters)) {
              scatters.forEach(({ col, row }) => animateScatter(col, row));
            }
